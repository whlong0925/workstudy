...
//        FileInputStream fis = new FileInputStream("c:/opt/keystores/clientKeystore.jks");
        FileInputStream fis = new FileInputStream(this.getClass().getResource("rain.jks").getPath());
        ks.load(fis, password);
        fis.close();

        KeyStore.PrivateKeyEntry pkEntry = (KeyStore.PrivateKeyEntry) ks.getEntry("rain", new KeyStore.PasswordProtection("".toCharArray()));
...
//		FileInputStream fis = new FileInputStream("c:/opt/keystores/serviceKeystore.jks");
		...
		ks.load(fis, password);
		fis.close();
		KeyStore.TrustedCertificateEntry tcEntry = (KeyStore.TrustedCertificateEntry) ks.getEntry("rain-public", null);
